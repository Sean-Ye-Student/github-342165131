...
sounds = {"menu" : {"minim" : "Nitrome Music - Bad Ice-Cream (Menu).mp3", "repeat" : -1, "play_from_start" : True, "isolate" : True, "group" : 0},"game" : {"minim" : "Nitrome Music Bad Ice Cream In Game.mp3", "repeat" : -1, "play_from_start" : False, "isolate" : True, "group" : 0}, "win" : {"minim" : "Nitrome Music - Bad Ice-Cream (Win).mp3", "repeat" : -1, "play_from_start" : True, "isolate" : True, "group" : 0}, "draw" : {"minim" : "Nitrome Music - Bad Ice-Cream (Lose).mp3", "repeat" : -1, "play_from_start" : True, "isolate" : True, "group" : 0}, "finish" : {"minim" : "Fortnite default dance sound.mp3", "repeat" : -1, "play_from_start" : True, "isolate" : True, "group" : 0}}
...
images = [tuple([{"image" : {"size" : {"x" : 180, "y" : 180}, "pos" : {"x" : x * 200 + 10, "y" : y * 200 + 10}, "animation" : {"file_index" : "images/o/", "file_type" : ".png", "start" : 0, "total_frames" : 13, "frame_duration" : 0.09}}} for y in range(3)]) for x in range(3)]
...
title = "tic tac toe"
title_images = tuple({"image" : {"size" : {"x" : 100, "y" : 100}, "pos" : {"x" : 100 * i - 100 * (int(i/4) * 2.5), "y" : 100 * int(i/4)}, "animation" : {"file_index" : "images/" + title[i] + "/", "file_type" : ".png", "start" : 0, "total_frames" : 13, "frame_duration" : 0.09}}} if c != " " else None for i, c in enumerate(title))
...
instructions = [{"text" : { "font" : "fonts/comic.ttf", "fill" : {"r" : 0, "g" : 0, "b" : 255, "a" : 255}, "word" : phrase[0], "size" : 25, "pos" : {"x" : phrase[1], "y" : 520 + i * 20, "z" : 0}}} for i, phrase in enumerate(phrases)]
def PLAY():
    global game_started
    game_started = True
    print(game_started)
...
def setup():
    size(600,600)
    minim = Minim(this)
    for ky in sounds:#Loads the minim objects into the sound dictionary
...
def WINSETUP():
    global winner_image, message, score_message, score, game_ended, reset, prevent_input
    PlaySound("finish", ("minim", "repeat", "play_from_start", "isolate", "group"))
    winner_image["image"]["animation"]["start"], message["text"]["startshow"] = time.time(), time.time()
    ...
    score[True], score[False], score_message["text"]["word"] = 0, 0, ""
    game_ended, reset, prevent_input = True, time.time() + 7, time.time() + 8
    
def RESETGAME():
    global game_ended, game_started, score_message, grid, turn, reset
    if game_ended:
        game_ended = False
        game_started = False
        return
    score_message["text"]["word"] = str(score[True]) + ":" + str(score[False]) 
    grid, turn, reset = [[None, None, None] for i in range(3)], True, -1
    
def ENDGAME(winner):
    global score, score_message, message, reset, prevent_input
    PlaySound("win" if winner != None else "draw", ("minim", "repeat", "play_from_start", "isolate", "group"))
    score_message["text"]["word"] = str(score[True]) + ":" + str(score[False]) 
    message["text"]["fill"]["r"], message["text"]["fill"]["g"], message["text"]["fill"]["b"] = (map(None, ((250, 50, 50) if winner else (50, 250, 50))) if winner != None else (69, 69, 69))
    message["text"]["word"] = ("Player " + ("X" if winner else "O") + " has Won!") if winner != None else "Both Players Draw!"
    if winner != None: 
        score[winner] += 1
    message["text"]["startshow"], reset, prevent_input = time.time(), time.time() + 5, time.time() + 6
...
def DRAWGAME():
    ...
    for x in range(len(images)):
        for y in range(len(images[x])): 
            images[x][y]["image"]["animation"]["file_index"] = "images/x/" if grid[x][y] else "images/o/"
...
def TURNLOGIC():
    ...
    for combo in combos:
        ...
        for coor in combo:
            if turn == grid[coor[0]][coor[1]] or grid[coor[0]][coor[1]] == None:
                ...
                break
        if t:
            ENDGAME(not(turn)) #The turn already became the next players turn, so the previous turn player, won
            return
    blanks = False
    for row in grid:
        if None in row:
    ...
    if not(blanks):
        ENDGAME(None) #Was a draw, so none of the players won
        return
def draw():
    DRAWGAME()#Will draw the board and the game pieces
    
    if game_started == False: 
        RENDERMENU()#If the game has not started, only load the menu
        return
        
        
    DRAWUI()#Will draw in game userinterface such as the score
    
    if reset > 0: #Make sure things are still being drawn
        if time.time() > reset:#Board is being reset, players cannot interact at this moment
            if score[True] >= max(max_score, 1) or score[False] >= max(max_score, 1):#If the score is greater or equal to the target score, trigger the win animation
                WINSETUP()
                return
            RESETGAME()#If the game is still going on, reset the board
        else:
            return
        
    PlaySound("game", ("minim", "repeat", "play_from_start", "isolate", "group")) #Plays the in game soundtrack
    if mousePressed and mouseButton == LEFT and (prevent_input < 0 or time.time() > prevent_input):#checks if a valid input was entered. Also prevents holding down.
        TURNLOGIC()#Runs logic for the players turn
